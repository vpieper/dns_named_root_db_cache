...
# -f : Fail fast (returns an error code if the server gives a 404/500 instead of downloading a useless error page)
# -s : Silent (hides the progress bar so it doesn't clutter your cron logs)
# -S : Show error (if it DOES fail, this ensures the actual error message is still output so we can capture it)
...
# 4. Apply the updates
# If the script makes it to this line, both downloads were 100% successful.
# We can now safely overwrite the live production files.
...
if /usr/sbin/rndc reload > /tmp/rndc_out.log 2>&1; then
    ...
    # The files downloaded, but BIND refused to reload. (This usually happens if you have a syntax error elsewhere in named.conf).
